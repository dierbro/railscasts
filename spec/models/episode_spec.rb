...
describe Episode do
  it "finds published" do
    a = create(:episode, :published_at => 2.weeks.ago)
    b = create(:episode, :published_at => 2.weeks.from_now)
  ...
  it "sorts recent episodes in descending order" do
    Episode.delete_all
    e1 = create(:episode, :position => 1)
    e2 = create(:episode, :position => 2)
    Episode.recent.should eq([e2, e1])
  end

  it "assigns tags to episodes" do
    episode = create(:episode, :tag_names => 'foo bar')
  ...
  it "knows if it's the last published episode" do
    a = create(:episode, :published_at => 2.weeks.ago)
    b = create(:episode, :published_at => 1.week.ago)
    c = create(:episode, :published_at => 2.weeks.from_now)
  ...
  it "has files with file sizes" do
    episode = create(:episode, :name => "Hello world", :file_sizes => {"zip" => "12345"})
  ...
  it "sets file size to zero when unknown" do
    episode = create(:episode, :name => "Hello world", :file_sizes => nil)
  ...
  it "has a full name which includes position" do
    Episode.delete_all
    create(:episode, :position => 123, :name => "Foo Bar").full_name.should eq('#123 Foo Bar')
  end

  it "knows the next and previous episode based on position" do
    Episode.delete_all
    e1 = create(:episode, :position => 1)
    e2 = create(:episode, :position => 6)
  ...
  describe "primitive search" do
    ...
    it "looks in name, description, and notes" do
      e1 = create(:episode, :name => 'foo', :description => 'bar', :notes => 'baz', :published_at => 2.weeks.ago)
      e2 = create(:episode, :name => 'foo test bar', :description => 'baz', :published_at => 2.weeks.ago)
      e3 = create(:episode, :name => 'foo', :published_at => 2.weeks.ago)
      Episode.search_published('foo bar baz').should eq([e1, e2])
    end

    it "does not find unpublished" do
      e1 = create(:episode, :name => 'foo', :published_at => 2.weeks.ago)
      e2 = create(:episode, :name => 'foo', :published_at => 2.weeks.from_now)
