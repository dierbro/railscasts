...
describe "Ability" do
  describe "as guest" do
    ...
    it "can only view episodes which are published" do
      ...
      @ability.should be_able_to(:show, build(:episode, :published_at => 2.days.ago))
      @ability.should_not be_able_to(:show, build(:episode, :published_at => 2.days.from_now))
  ...
  describe "as normal user" do
    ...
    it "can create comments and update/destroy within 15 minutes if he owns them" do
      ...
      @ability.should be_able_to(:update, create(:comment, :user => @user, :created_at => 10.minutes.ago))
      @ability.should_not be_able_to(:update, create(:comment, :user => @user, :created_at => 20.minutes.ago))
      @ability.should be_able_to(:destroy, create(:comment, :user => @user, :created_at => 10.minutes.ago))
      @ability.should_not be_able_to(:destroy, create(:comment, :user => @user, :created_at => 20.minutes.ago))
      @ability.should_not be_able_to(:destroy, create(:comment, :user => User.new, :created_at => 10.minutes.ago))
    end
  ...
  describe "as banned user" do
    before(:each) do
      @user = create(:user, :banned_at => Time.now)
      @ability = Ability.new(@user)
    end
  ...
  describe "as moderator" do
    before(:each) do
      @user = create(:user, :moderator => true)
      @ability = Ability.new(@user)
    end
    ...
    it "can list, update and destroy any comments" do
      @ability.should be_able_to(:update, create(:comment, :user => User.new, :created_at => 20.minutes.ago))
      @ability.should be_able_to(:destroy, create(:comment, :user => User.new, :created_at => 20.minutes.ago))
      @ability.should be_able_to(:index, :comments)
    end

    it "can view episodes which are not yet published" do
      @ability.should be_able_to(:index, build(:episode, :published_at => 2.days.from_now))
      @ability.should be_able_to(:show, build(:episode, :published_at => 2.days.from_now))
    end
