...
describe "Episodes request" do
  it "lists published" do
    create(:episode, :name => "Blast from the Past", :published_at => 2.days.ago)
    create(:episode, :name => "Back to the Future", :published_at => 2.days.from_now)
    visit episodes_path
  ...
  it "has different views" do
    create(:episode, :name => "Blast from the Past", :description => "I am from 1960", :published_at => 2.days.ago)
    visit episodes_path
  ...
  it "searches by name" do
    create(:episode, :name => "Blast from the Past")
    create(:episode, :name => "Back to the Future")
    visit episodes_path
  ...
  it "filters by tag" do
    create(:episode, :name => "Blast from the Past", :tags => [build(:tag, :name => "Oldtimes")])
    create(:episode, :name => "Back to the Future")
    visit episodes_path
    click_on "Oldtimes"
    page.should have_content("Blast from the Past")
  ...
  it "contains show notes, comments, and similar episodes" do
    episode = create(:episode, :name => "Blast from the Past", :notes => "Show notes!", :position => 1)
    create(:comment, :content => "Hello world", :episode => episode)
  ...
  it "edits an episode show notes as moderator" do
    FakeWeb.register_uri(:head, /.*/, :content_length => 123)
    login create(:user, :moderator => true)
